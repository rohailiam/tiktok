...
app.post('/youtube', async (req, res) => {
  ...
  .then(async (text) => {
    ...
    for(let i = 0; i < dataScript.length; i++) {
      let data = dataScript.get(i).children[0] ? dataScript.get(i).children[0].data ? dataScript.get(i).children[0].data.includes('ytInitialData') ? dataScript.get(i).children[0].data : null : null : null;
      ...
      let views = jsonData.engagementPanels[1].engagementPanelSectionListRenderer.content.structuredDescriptionContentRenderer.items[0].videoDescriptionHeaderRenderer.views.simpleText
      let channel = jsonData.engagementPanels[1].engagementPanelSectionListRenderer.content.structuredDescriptionContentRenderer.items[0].videoDescriptionHeaderRenderer.channel.simpleText
      let publishDate = jsonData.engagementPanels[1].engagementPanelSectionListRenderer.content.structuredDescriptionContentRenderer.items[0].videoDescriptionHeaderRenderer.publishDate.simpleText
      if(views) {
        views = views.split(' ')[0]
      }
      let likes = overlayData.likeButton.likeButtonRenderer.likeCount
      let title = overlayData.reelPlayerHeaderSupportedRenderers.reelPlayerHeaderRenderer.reelTitleText.runs[0].text
      let comments = overlayData.viewCommentsButton.buttonRenderer.accessibility.label
      if(comments) {
      ...
      // let jsonData = cleanData.slice(0, scriptIndex);
      // console.log(scriptIndex)
      // console.log(jsonData)
      // jsonData = jsonData.replace(')()', '')
      // jsonData = JSON.parse(jsonData)
      // console.log(jsonData)
      // let videoDetails = jsonData.contents.twoColumnWatchNextResults.results.results.contents[1].videoSecondaryInfoRenderer
